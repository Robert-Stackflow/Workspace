...
void Widget::loadItemFunction(QString& groupName)
{
    if(newKey!=NULL&&newKey->flag)
    {
        ...
        groups[index]->keys<<newKey;
        groups[index]->lastEditTime=newKey->lastEditTime;
...
void Widget::newItemSlot()
{
    ...
    newitemdialog->exec();
    newItemFunction(groupName);
}
void Widget::newItemFunction(QString& groupName)
{
    if(newKey!=NULL&&newKey->flag)
    {
        int index=0;
        ...
        {
            QString sql=QString("update groups set count='%1',lastEditTime='%2' where name='%3' ").arg(groups[index]->keys.size()).arg(groups[index]->lastEditTime.toString("yyyy-MM-dd hh:mm:ss")).arg(groups[index]->name);
            query.exec(sql);
            switch(newKey->type)
            {
            case 0:
                query.prepare("insert into "+groupName+" (type,name,nickName,id,password,mobile,mail,website,remark,createTime,lastEditTime)"
            ...
            updateTableWidgetView(index);
            if(autofillInfo!=NULL&&!autofillInfo->mails.contains(newKey->mail))
                autofillInfo->mails<<newKey->mail,autofillInfo->mailAliases<<"";
            if(autofillInfo!=NULL&&newKey->type==1)
                autofillInfo->mailAliases[autofillInfo->mails.indexOf(newKey->mail)]=newKey->subName;
            if(autofillInfo!=NULL&&!autofillInfo->mobiles.contains(newKey->mobile))
                autofillInfo->mobiles<<newKey->mobile;
            if(autofillInfo!=NULL&&newKey->type==1&&!autofillInfo->websites.contains(newKey->website))
                autofillInfo->websites<<newKey->website;
            updateAutofillInfo();
            data.close();
        }
    }
...
void Widget::removeItemSlot()
{
    ...
    for(int i=selectRows.size()-1;i>=0;i--)
    {
        ...
        message+=groups[stackedWidget->currentIndex()]->keys[row]->name;
        message+="\n";
    }
    ...
    if(choice==QMessageBox::Yes)
    {
        for(int i=selectRows.size()-1;i>=0;i--)
        {
            ...
            else
            {
                ...
                QString sql = QString("delete from '%1' where createTime = '%2' ").arg(groups[index]->name).arg(groups[index]->keys[row]->createTime.toString("yyyy-MM-dd hh:mm:ss"));
                query.exec(sql);
                groups[index]->keys.removeAt(row);
                groups[index]->lastEditTime=QDateTime::currentDateTime();
                sql=QString("update groups set count='%1',lastEditTime='%2' where name='%3' ").arg(groups[index]->keys.size()).arg(groups[index]->lastEditTime.toString("yyyy-MM-dd hh:mm:ss")).arg(groups[index]->name);
                query.exec(sql);
...
void Widget::openWebsiteSlot()
{
    ...
    if(row==-1)
        return;
    QDesktopServices::openUrl(QUrl(groups[stackedWidget->currentIndex()]->keys[row]->website, QUrl::TolerantMode));
}
void Widget::copyItemInfoSlot()
{
    int x=cursorPos->x();
    int y=cursorPos->y();
    QModelIndex index = tableWidgets[stackedWidget->currentIndex()]->indexAt(QPoint(x, y));
    ...
    clip->setText(groups[stackedWidget->currentIndex()]->keys[row]->toString());
}
...
{
    int type=groups[stackedWidget->currentIndex()]->type;
    int certainColumn=0;
    switch(type)
    {
    case 0:
    ...
    if(column==certainColumn)
    {
        if(tableWidgets[stackedWidget->currentIndex()]->item(row,column)->text()=="******")
            tableWidgets[stackedWidget->currentIndex()]->item(row,column)->setText(groups[stackedWidget->currentIndex()]->keys[row]->password);
        else
            tableWidgets[stackedWidget->currentIndex()]->item(row,column)->setText("******");
    }
}
void Widget::moveItemSlot()
{
    ...
    std::sort(selectRows.begin(),selectRows.end());
    QString message=QString("是否将已选条目从 '%1' 移动到 '%2' :\n").arg(groups[stackedWidget->currentIndex()]->name).arg(send->text());
    for(int i=selectRows.size()-1;i>=0;i--)
    {
        int row=selectRows[i];
        message+=groups[stackedWidget->currentIndex()]->keys[row]->name;
    ...
    if(choice==QMessageBox::Yes)
    {
        for(int i=selectRows.size()-1;i>=0;i--)
        {
            ...
            else
            {
                ...
                int index=stackedWidget->currentIndex();
                newKey=groups[index]->keys[row];
                newItemFunction(newName);
                //从原表中删除
                QString sql = QString("delete from '%1' where createTime = '%2' ").arg(groups[index]->name).arg(groups[index]->keys[row]->createTime.toString("yyyy-MM-dd hh:mm:ss"));
                query.exec(sql);
                groups[index]->keys.removeAt(row);
...
void Widget::copyItemSlot()
{
    ...
    if(choice==QMessageBox::Yes)
    {
        for(int i=selectRows.size()-1;i>=0;i--)
        {
            ...
            int index=stackedWidget->currentIndex();
            newKey=new ITEM;
            newKey->createTime=groups[index]->keys[row]->createTime;
            newKey->lastEditTime=groups[index]->keys[row]->lastEditTime;
            newKey->type=groups[index]->keys[row]->type;
            newKey->flag=1;
            newKey->id=groups[index]->keys[row]->id;
            newKey->mail=groups[index]->keys[row]->mail;
            newKey->mobile=groups[index]->keys[row]->mobile;
            newKey->name=groups[index]->keys[row]->name;
            newKey->nickName=groups[index]->keys[row]->nickName;
            newKey->password=groups[index]->keys[row]->password;
            newKey->remark=groups[index]->keys[row]->remark;
            newKey->subName=groups[index]->keys[row]->subName;
            newKey->website=groups[index]->keys[row]->website;
            newItemFunction(newName);
        }
    }
}
